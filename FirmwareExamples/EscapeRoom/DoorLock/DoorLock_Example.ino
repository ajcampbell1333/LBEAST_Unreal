/*
 * LBEAST Door Lock Example Firmware
 * 
 * Example firmware for controlling door locks/latches in escape room installations.
 * Demonstrates wireless communication with Unreal Engine using LBEAST protocol.
 * 
 * Functionality:
 ...
 * 
 * This example uses the LBEAST_Wireless_RX.h template for receiving unlock commands.
...
void LBEAST_SendBool_TX(uint8_t channel, bool value);
uint8_t LBEAST_CalculateCRC_TX(uint8_t* data, int length);
...
void unlockDoor(int doorIndex) {
  ...
  LBEAST_SendBool_TX(doorIndex, true);
...
void lockDoor(int doorIndex) {
  ...
  LBEAST_SendBool_TX(doorIndex, false);
...
void LBEAST_SendBool_TX(uint8_t channel, bool value) {
  ...
  packet[2] = channel;
  packet[3] = value ? 1 : 0;
  packet[4] = LBEAST_CalculateCRC_TX(packet, 4);
  
  // Use the RX UDP object to send (it's already initialized)
  LBEAST_UDP.beginPacket(unrealIP, unrealPort);
