/*
 * LBEAST Superhero Flight Experience ECU
 * 
 ...
 * 
 * Communication Protocol: Binary LBEAST protocol over UDP (WiFi/Ethernet)
 * 
...
// Include LBEAST protocol templates
...
void setup() {
  ...
  LBEAST_Wireless_Init(ssid, password, unrealPort);
  // TX: Configure target IP/port for sending telemetry (WiFi already connected)
  LBEAST_TargetIP = unrealIP;
  LBEAST_TargetPort = unrealPort;
  LBEAST_Initialized = true;  // Mark TX as initialized (WiFi already connected by RX init)
...
void loop() {
  // Process incoming UDP commands (from LBEAST_Wireless_RX.h)
...
void SendWinchState() {
  ...
  LBEAST_SendBytes(310, structBytes, structSize);
}
...
void SendTelemetry() {
  ...
  LBEAST_SendBytes(311, structBytes, structSize);
}
