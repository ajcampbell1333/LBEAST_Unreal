 ...
 * 
 * This example uses the LBEAST_Wireless_RX.h and LBEAST_Wireless_TX.h templates.
...
void SendTelemetry() {
  ...
  LBEAST_SendFloat(30 + STATION_ID, solenoidTemperatures[activeSolenoidID]);
  
  // Active solenoid ID (Ch 40+n, if redundant)
  if (numSolenoids > 1) {
    LBEAST_SendInt32(40 + STATION_ID, (int32_t)activeSolenoidID);
  }
  
  // Total solenoid count (Ch 50+n, if redundant)
  if (numSolenoids > 1) {
    LBEAST_SendInt32(50 + STATION_ID, (int32_t)numSolenoids);
  }
  ...
  for (uint8_t i = 0; i < numSolenoids && i < 2; i++) {  // Send first 2 solenoids
    LBEAST_SendFloat(60 + STATION_ID * 2 + i, solenoidTemperatures[i]);
  }
  
  // Driver module temperature (Ch 70+n)
  LBEAST_SendFloat(70 + STATION_ID, driverModuleTemperature);
  
  // Fire command active (Ch 80+n)
  LBEAST_SendBool(80 + STATION_ID, fireCommandActive);
  
  // Thermal shutdown (Ch 90+n)
  LBEAST_SendBool(90 + STATION_ID, thermalShutdown);
  
  // PWM throttle (Ch 95+n)
  LBEAST_SendFloat(95 + STATION_ID, currentPWMThrottle);
  
  // Send gun pose struct (Struct 150+n) - placeholder
  // TODO: Implement pose struct transmission when tracker integration complete
  // LBEAST_SendBytes(150 + STATION_ID, (uint8_t*)&gunPose, sizeof(FGunPose));
}
