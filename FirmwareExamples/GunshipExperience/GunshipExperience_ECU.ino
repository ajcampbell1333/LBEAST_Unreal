...
void ProcessGunECUTelemetry() {
  ...
  if (buffer[0] != 0xAA) return;  // LBEAST start marker
  
  // Get sender IP (for station identification)
  IPAddress senderIP = gunECU_UDP.remoteIP();
  
  // Parse LBEAST packet: [0xAA][Type][Channel][Payload...][CRC]
...
void SendGunButtonEvents() {
  ...
  LBEAST_SendBytes(310, (uint8_t*)&buttonEvents, sizeof(FGunButtonEvents));
}
...
void SendGunTelemetry() {
  ...
  LBEAST_SendBytes(311, (uint8_t*)&telemetry, sizeof(FGunTelemetry));
}
