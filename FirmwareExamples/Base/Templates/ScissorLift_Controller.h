/*
 * LBEAST Scissor Lift Controller Header
 * 
...
public:
  ...
  // Command handlers - call these from your LBEAST_HandleFloat/Bool functions
  ...
  // Manual control (optional - for direct control without LBEAST commands)
...
void ScissorLiftController::begin(const ScissorLiftConfig& cfg) {
  ...
  if (config.useCANBus) {
    ...
    if (!LBEAST_CAN_Init(config.canBaudRate, config.canCSPin)) {
...
void ScissorLiftController::setEmergencyStop(bool stop) {
  if (stop && !emergencyStop) {
    ...
    if (config.useCANBus) {
      LBEAST_CAN_SendLiftEmergencyStop(true, config.canIdControl);
    }
  }
  emergencyStop = stop;
  if (!stop) {
    ...
    if (config.useCANBus) {
      LBEAST_CAN_SendLiftEmergencyStop(false, config.canIdControl);
    }
  }
}
...
void ScissorLiftController::updatePositionSensor() {
  if (config.useCANFeedback) {
    ...
    // This would require implementing CAN message reception in LBEAST_CAN.h
...
void ScissorLiftController::executeForwardReverseMotion() {
  ...
  if (config.useCANBus) {
    ...
    LBEAST_CAN_SendLiftJoystickCommand(verticalCommand, forwardCommand, config.canIdJoystick);
...
void ScissorLiftController::stopMotor() {
  if (config.useCANBus) {
    ...
    LBEAST_CAN_SendLiftJoystickCommand(0.0f, 0.0f, config.canIdJoystick);
  } else {
    // Direct GPIO mode
    digitalWrite(config.motorUpPin, LOW);
    digitalWrite(config.motorDownPin, LOW);
...
void ScissorLiftController::stopForwardReverse() {
  if (config.enableForwardReverse) {
    if (config.useCANBus) {
      ...
      LBEAST_CAN_SendLiftJoystickCommand(verticalCommand, 0.0f, config.canIdJoystick);
    } else {
      // Direct GPIO mode
      digitalWrite(config.motorForwardPin, LOW);
      digitalWrite(config.motorReversePin, LOW);
